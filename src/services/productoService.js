...
export const productoService = {
  ...
  obtenerPorCategoria: async (categoria) => {
    const response = await productoApi.get(`/api/v1/productos/categoria/${categoria}`);
    return response.data;
  },
  ...
  obtenerInventario: async (productoId) => {
    const response = await productoApi.get(`/api/v1/inventario/producto/${productoId}`);
    return response.data;
  },
  ...
  actualizarStock: async (productoId, cantidad) => {
    const response = await productoApi.put(`/api/v1/inventario/${productoId}/stock`, { cantidad });
    return response.data;
  },
};
