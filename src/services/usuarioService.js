...
export const usuarioService = {
  ...
  login: async (email, contrasena) => {
    const response = await usuarioApi.post('/api/v1/auth/login', {
      email,
      contrasena,
    });
  ...
  actualizar: async (id, datosUsuario) => {
    const response = await usuarioApi.put(`/api/v1/usuarios/${id}/actualizar`, datosUsuario);
    return response.data;
  },
  ...
  cambiarEstado: async (id, estado) => {
    const response = await usuarioApi.patch(`/api/v1/usuarios/${id}/estado`, estado);
    return response.data;
  },
